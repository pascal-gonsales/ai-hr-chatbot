...
// is fronted by a CAPTCHA and edge rate limiting. Set the env var
...
export async function POST(request: Request) {
  if (!PUBLIC_ENABLED) {
    // Fail closed: do not accept submissions until a CAPTCHA is in place.
  ...
  // 'pending' (migration 005) means a concurrent duplicate pending insert
  // raises a 23505 unique violation. We use a plain .insert() and rely on
  // that DB constraint, closing the TOCTOU window between the count check
  // and the write.
  ...
  // 23505 = unique_violation. Treat as a soft success so we do not leak the
