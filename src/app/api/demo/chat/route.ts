...
// - Uses the Alex Demo fixture employee (src/lib/demo/fixtures.ts)
...
// the production auth/RLS path remains untouched.
...
export async function POST(request: Request) {
  ...
  // Anthropic key is required at runtime (set in the deployment env)
  ...
  // Validate + bound history. Drop client-supplied history beyond the most
  // recent entries; reject invalid roles/content; cap total history budget.
  ...
  // Build system prompt for the fixture employee (Alex Demo at Northstar Demo Bistro)
  const systemPrompt =
    buildSystemPrompt({
      ...
      try {
        ...
        while (maxIterations > 0) {
          ...
          const response = await anthropic.messages.create({
            ...
            // tool-use loop. The production chat route uses a current Sonnet.
