...
export default function BottomNav({ isAdmin, lang }: BottomNavProps) {
  const pathname = usePathname()

  const navItems = [
    { href: '/chat', labelKey: 'nav.chat', icon: ChatIcon },
    { href: '/tips', labelKey: 'nav.tips', icon: TipsIcon },
  ]

  const items = isAdmin
    ? [...navItems, { href: '/admin', labelKey: 'nav.admin', icon: AdminIcon }]
    : navItems

  return (
    <nav className="flex items-center justify-around bg-zinc-900 border-t border-zinc-800 px-2 py-1 safe-area-bottom">
      {items.map((item) => {
